...
/// `symcrypt_provider_with_cipher_suites` provides a way to set up an custom config using a `symcrypt` crypto backend.
///
/// `provided_cipher_suites` takes in an optional `Vec<>` of `[SupportedCipherSuites]`
/// The supplied arguments for `provided_cipher_suite` and `provided_kx_group` will be used when when negotiating the TLS cipher suite;
